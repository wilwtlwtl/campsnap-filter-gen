...
if presets:
    ...
    if selected != "（選択しない）":
        ...
        with col_b:
            if st.button("✅ このプリセットを使う", width="stretch", type="primary"):
            ...
            def _render_group(names):
                for row_start in range(0, len(names), GALLERY_COLS):
                    ...
                    for col, name in zip(cols, row_names):
                        with col:
                            st.image(previews[name], width="stretch")
                            st.caption(name)
                            if st.button("✅ 使う", key=f"gallery_btn_{name}", width="stretch"):
...
if uploaded:
    ...
    st.image(st.session_state.target_img, width="stretch")
...
with st.expander("📁 V105で撮った写真もあれば、より正確に解析できます（任意）", expanded=False):
    ...
    if base_uploaded:
        ...
        if st.session_state._upload_base_id != file_id:
            ...
            st.session_state.analyzed = False   # ファイルが変わったときだけリセット
        st.image(st.session_state.base_img, caption="V105の写真（元画像）", width="stretch")
...
if st.session_state.analyzed:
    ...
    with st.expander("🖼️ 自分の写真でフィルターを試す（任意）", expanded=False):
        ...
        if st.session_state.preview_img:
            st.image(st.session_state.preview_img, width="stretch")
    ...
    if preview_src is not None:
        if show_sim:
            ...
            with c1:
                st.caption("元の写真")
                st.image(preview_src, width="stretch")
            with c2:
                st.caption(f"フィルター適用後（{strength}%）")
                st.image(apply_filter(preview_src, p_blended), width="stretch")
            with c3:
                st.caption("V105実機シミュレーション")
                with st.spinner(""):
                    st.image(simulate_v105(preview_src, p_blended), width="stretch")
            st.caption(
    ...
    with st.expander("📖 SDカードへの入れ方", expanded=False):
        st.markdown("""
1. SDカードをパソコンに挿す
2. ダウンロードした `.flt` ファイルをSDカードの**ルート直下**（一番上の階層）にコピーする
...
with st.sidebar:
    ...
    ENGINE_DESC = {
        "高精度モード": (
            "明るさ・色味・明暗のバランスを細かく分けて丁寧に読み取り、"
            "理想の写真に近づけます。\n\n"
            "📷 **V105で撮った写真**と**理想の写真**の\n"
            "**2枚**が必要です。\n\n"
            "精度が最も高く、一番おすすめのモードです。"
        ),
        "標準モード": (
            "写真全体の色の雰囲気をまとめて読み取り、"
            "理想の色合いに近づけます。\n\n"
